...
const NavBarThree = () => {
  ...
  const yesterday = { date:format(subDays(new Date(), 1), "do MMMM"), day: format(subDays(new Date(), 1), "EEEE")
}
  const twoDaysAgo = { date: format(subDays(new Date(), 2), "do MMMM") , day: format(subDays(new Date(), 2), "EEEE")}
  const threeDaysAgo = { date: format(subDays(new Date(), 3), "do MMMM"), day: format(subDays(new Date(), 3), "EEEE")}
  ...
  return (
    <>
      <Stack direction={["column", "column", "row"]} spacing="5" p="10">
        <Link href="/" passHref>
          <DateCard day={threeDaysAgo.}>{threeDaysAgo.date}</DateCard>
        </Link>
        <Link href="/" passHref>
